...
import { ADMIN_NUMBER, AI_MODEL } from "./config";
...
export const performAudit = onCall(async (request) => {
  ...
  try {
    ...
    await db.collection("leads").add({
      businessName, email: clientEmail, score: analysis.score, timestamp: FieldValue.serverTimestamp()
    });
...
export const hunterChat = onCall(async (request) => {
  ...
  try {
    const prompt = `You are Smart Marketing Chat for Happy Hunter Digital using ${AI_MODEL}. NO Markdown asterisks. Mission: Stop SA SMEs from being Ghosts to AI. Tool: happyhunterdigital.com/audit. Contact: WhatsApp +27(0) 60 101 6673.`;
