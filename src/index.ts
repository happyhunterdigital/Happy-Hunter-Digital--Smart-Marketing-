...
// CORS whitelist - must include EXACT origins including www
...
// Secure Audit Function with proper CORS and secrets binding
export const performForensicAudit = onCall(
  {
    region: REGION,
    // Secrets must be explicitly bound to the function
    secrets: ["GEMINI_API_KEY", "PLACES_API_KEY"],
    cors: ALLOWED_ORIGINS,
    timeoutSeconds: 30,
    memory: "512MiB", // Increased from 256MiB for Gemini API calls
    minInstances: 0,
    maxInstances: 10
  },
  async (request) => {
...
// Secure Chat Proxy with proper error handling
export const hunterChatProxy = onCall(
  ...
  async (request) => {
    ...
    logger.info("Chat request received", { promptLength: prompt.length });
...
CONTEXT: You are speaking to a potential client on the website.
