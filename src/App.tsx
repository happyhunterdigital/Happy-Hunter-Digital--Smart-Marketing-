...
function App() {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-yellow-500 selection:text-black pt-12">
      ...
      {/* CAPSULE NAVBAR - Shifted down to top-16 to avoid the thicker ticker */}
        ...
        <div className="fixed top-32 left-4 right-4 z-[100] bg-black/95 backdrop-blur-2xl border border-gray-800 rounded-3xl p-8 shadow-2xl animate-fade-in lg:hidden">
          <div className="flex flex-col space-y-6 text-center">
            {["Services", "Earned Media", "Intelligence", "Founders", "FAQ"].map((link) => (
              <Link key={link} to={`/${link.toLowerCase().replace(" ", "-")}`} className="text-sm font-black tracking-widest uppercase hover:text-yellow-500 text-white">{link}</Link>
            ))}
            <div className="pt-4 border-t border-white/10">
              <Link to="/audit" className="inline-block bg-yellow-500 text-black px-8 py-3 rounded-full text-xs font-black tracking-widest w-full uppercase">Start Audit</Link>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="container mx-auto px-6 grid md:grid-cols-2 lg:grid-cols-4 gap-16">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4 text-xs">
              <a href="mailto:motsumitl@happyhunterdigital.com" className="flex items-center gap-3 text-gray-300 hover:text-yellow-500 transition-all font-medium"><Mail size={16} className="text-yellow-500"/> motsumitl@happyhunterdigital.com</a>
              <a href="https://wa.me/27601016673" className="flex items-center gap-3 text-gray-300 hover:text-yellow-500 transition-all font-medium"><Phone size={16} className="text-yellow-500"/> +27 (0) 60 101 6673</a>
              <a href="https://www.happyhunterdigital.com" className="flex items-center gap-3 text-gray-300 hover:text-yellow-500 transition-all font-medium"><Globe size={16} className="text-yellow-500"/> www.happyhunterdigital.com</a>
            </div>
          </div>
