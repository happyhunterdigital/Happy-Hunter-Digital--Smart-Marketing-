...
import { Shield, Clock, FileText, Phone, Mail, MapPin, Users, Loader2, LogOut, Globe, AlertOctagon, Download, Zap } from 'lucide-react';
      ...
      <div className="min-h-screen flex items-center justify-center bg-slate-950 px-6">
        <div className="max-w-md w-full p-12 border border-slate-800 rounded-[3rem] bg-slate-900/30 backdrop-blur-xl shadow-2xl text-center relative overflow-hidden">
          <div className="flex justify-center mb-8 relative z-10">
            <div className="p-4 bg-yellow-500 rounded-2xl text-slate-950 shadow-[0_0_30px_rgba(250,204,21,0.2)] animate-pulse"><Shield size={32} /></div>
          </div>
          ...
          <button onClick={handleGoogleLogin} disabled={isAuthorizing} className="w-full bg-white text-slate-950 py-4 rounded-xl font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-3 hover:bg-yellow-500 transition-all shadow-xl group relative z-10">
    ...
    <div className="pt-40 pb-20 px-6 max-w-7xl mx-auto min-h-screen font-sans">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-16 gap-8 animate-fade-in">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={handleLogout} className="p-4 bg-slate-900 border border-slate-800 rounded-2xl text-slate-500 hover:text-red-500 transition-all hover:scale-105"><LogOut size={20} /></button>
        </div>
      </div>
      ...
      <div className="grid gap-6">
        {loading ? (
          <div className="flex flex-col items-center py-20 space-y-4 font-black uppercase text-[10px] tracking-widest">
            <Loader2 className="animate-spin text-yellow-500" size={40} /><p className="text-slate-500 tracking-[0.4em]">establishing signal...</p>
          </div>
        ) : leads.map((lead) => (
          <div key={lead.id} className="p-10 border border-slate-800 rounded-[3.5rem] bg-slate-900/20 hover:border-yellow-500/20 transition-all group animate-fade-in relative overflow-hidden">
            <div className="flex flex-col lg:flex-row justify-between gap-8">
              ...
              <div className="text-left lg:text-right shrink-0">
                 ...
                 <p className="text-5xl font-black text-yellow-500 leading-none">{lead.score || "??"}<span className="text-xs text-slate-800">/100</span></p>
                 <p className="text-slate-700 text-[8px] font-black uppercase tracking-widest mt-4"><Clock size={10} className="inline mr-1" /> {lead.timestamp?.toDate().toLocaleString()}</p>
              </div>
            </div>
            <div className="mt-8 p-8 bg-slate-950/50 rounded-[2.5rem] border border-slate-800 relative">
              <div className="flex items-center gap-2 mb-4 text-slate-600 font-black uppercase text-[8px] tracking-[0.3em]"><FileText size={12} className="text-yellow-500" /> Strategic Analysis Protocol Analysis</div>
              <div className="text-slate-400 text-xs leading-relaxed whitespace-pre-wrap font-medium line-clamp-4">{lead.analysis}</div>
            </div>
          </div>
        ))}
      </div>
