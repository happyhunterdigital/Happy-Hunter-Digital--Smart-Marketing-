...
export default function Audit() {
  ...
  async function runAudit() {
    ...
    const aiResponse = await callHunterAI(`Analyze the digital entity for: ${bizName}. List 3 gaps and 1 strategy.`);
  ...
  return (
    <div className="pt-32 pb-20 px-6 max-w-5xl mx-auto min-h-screen text-center">
      ...
      {result && (
        <div className={`max-w-3xl mx-auto p-10 border rounded-[2.5rem] text-left animate-in fade-in ${result.includes("ERROR") ? 'border-red-500 bg-red-500/10' : 'border-slate-800 bg-slate-900/40'}`}>
          <div className="flex items-center gap-3 mb-8">
            {result.includes("ERROR") ? <AlertCircle className="text-red-500" /> : <ShieldCheck className="text-yellow-500" />}
            <h3 className="font-black uppercase tracking-widest text-[10px]">
              {result.includes("ERROR") ? 'System Diagnostic' : 'Strategic Analysis Protocol'}
            </h3>
          </div>
