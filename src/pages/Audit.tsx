...
export default function Audit() {
  ...
  const renderText = (text: string) => {
    ...
    return text.split('\n').map((line, i) => {
      ...
      if (line.includes('[FIX]')) return <div key={i} className="bg-yellow-500/10 border-l-4 border-yellow-500 p-8 my-10 text-white font-bold tracking-tight italic">Requirement: {line.replace('[FIX]', '')}</div>;
    ...
    <div className="pt-32 pb-20 px-6 max-w-6xl mx-auto min-h-screen">
      {step === 1 && (
        <div className="max-w-2xl mx-auto text-center animate-in fade-in zoom-in">
          ...
          <form onSubmit={(e) => { e.preventDefault(); setStep(2); }} className="space-y-4">
            <input required className="w-full bg-slate-900 border border-slate-800 p-6 pl-16 rounded-3xl text-white outline-none focus:border-yellow-500" placeholder="Business Name" value={formData.bizName} onChange={e => setFormData({...formData, bizName: e.target.value})} />
            ...
            <button type="submit" className="w-full bg-yellow-500 text-slate-950 py-6 rounded-3xl font-black text-xl uppercase shadow-xl">Assess Business</button>
          </form>
        </div>
      )}
      ...
      {step === 2 && (
        <div className="max-w-xl mx-auto p-12 border-2 border-slate-800 rounded-[3.5rem] bg-slate-900/40 shadow-2xl animate-in slide-in-from-bottom-10 text-center">
          ...
          <h3 className="text-3xl font-black uppercase text-white mb-8 tracking-tighter">Secure Results</h3>
          <form onSubmit={runAudit} className="space-y-4">
            ...
            <button disabled={loading} className="w-full bg-yellow-500 text-slate-950 py-6 rounded-2xl font-black uppercase shadow-xl">{loading ? <><Loader2 className="animate-spin" size={20}/> {loadingText}</> : "Initiate Smart Analysis"}</button>
          </form>
        </div>
      )}
      ...
      {step === 3 && (
        <div className="animate-in fade-in">
          <div className="bg-yellow-500 p-10 rounded-[3.5rem] mb-12 text-slate-950 flex flex-col md:flex-row items-center justify-between gap-10 shadow-2xl">
            <div className="text-center md:text-left"><h3 className="text-3xl font-black uppercase leading-none italic">The Verdict is Ready</h3><p className="font-bold">Protocol results retrieved from the Smart Marketing Graph.</p></div>
            <div className="flex gap-4">
              <button onClick={downloadPDF} className="bg-slate-950 text-white px-10 py-5 rounded-2xl font-black uppercase text-xs shadow-2xl hover:scale-105 transition-all"><Download size={20}/> Download PDF</button>
              <a href={`https://wa.me/27601016673?text=My Score: ${score}/100`} target="_blank" className="bg-green-600 text-white px-10 py-5 rounded-2xl font-black uppercase text-xs shadow-2xl hover:scale-105 transition-all"><MessageCircle size={20}/> WhatsApp Thabo</a>
            </div>
          </div>
          ...
          <div ref={reportRef} className="p-16 md:p-24 border-2 border-slate-800 rounded-[4rem] bg-slate-900/40 relative shadow-2xl overflow-hidden text-left">
             ...
             <div className="mt-32 p-20 bg-yellow-500 rounded-[3rem] text-slate-950 text-center shadow-2xl">
