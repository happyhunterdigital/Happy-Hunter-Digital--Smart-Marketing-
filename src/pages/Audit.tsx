...
export default function Audit() {
  ...
  const runAudit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (aiResponse.includes("SYSTEM_ERROR")) {
      ...
      if (db) {
        try {
          ...
          console.warn("Database sync delayed. Showing local results.");
        }
      }
      setStep(3);
    } catch (err) {
      setResult("Handshake failed. Please contact Thabo directly.");
      setStep(3);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="pt-32 pb-20 px-6 max-w-6xl mx-auto min-h-screen">
      {step === 1 && (
        <div className="max-w-2xl mx-auto text-center animate-in fade-in zoom-in">
          ...
          <form onSubmit={(e) => { e.preventDefault(); setStep(2); }} className="space-y-4">
            <input required className="w-full bg-slate-900 border border-slate-800 p-6 pl-16 rounded-3xl text-white outline-none focus:border-yellow-500 transition-all text-lg" placeholder="Business Name" value={formData.bizName} onChange={e => setFormData({...formData, bizName: e.target.value})} />
            <input required className="w-full bg-slate-900 border border-slate-800 p-6 pl-16 rounded-3xl text-white outline-none focus:border-yellow-500 transition-all text-lg" placeholder="City" value={formData.location} onChange={e => setFormData({...formData, location: e.target.value})} />
      ...
      {step === 2 && (
        <div className="max-w-xl mx-auto p-12 border border-slate-800 rounded-[3.5rem] bg-slate-900/40 shadow-2xl animate-in slide-in-from-bottom-10 text-center">
          ...
          <form onSubmit={runAudit} className="space-y-4">
            <input required className="w-full bg-slate-950 border border-slate-800 p-5 rounded-2xl text-white outline-none" placeholder="Full Name" value={formData.fullName} onChange={e => setFormData({...formData, fullName: e.target.value})} />
      ...
      {step === 3 && (
        <div className="animate-in fade-in">
          <div className="bg-yellow-500 p-10 rounded-[3.5rem] mb-12 text-slate-950 flex flex-col md:flex-row items-center justify-between gap-10 shadow-2xl">
            <div className="text-center md:text-left"><h3 className="text-3xl font-black uppercase leading-none italic tracking-tight underline decoration-slate-950/20">The Verdict is Ready</h3><p className="font-bold">I have attempted to dispatch your report to {formData.email}. Download the PDF below.</p></div>
          ...
          <div ref={reportRef} className="p-16 md:p-24 border-2 border-slate-800 rounded-[4rem] bg-slate-900/40 relative shadow-2xl overflow-hidden text-left">
             ...
             <div className="mt-32 p-16 bg-yellow-500 rounded-[3rem] text-slate-950 text-center">
               ...
               <a href="https://calendly.com/motsumitl/30min" className="bg-slate-950 text-white px-16 py-6 rounded-2xl font-black uppercase text-sm hover:bg-white transition-all shadow-2xl inline-block">Schedule Recovery Call</a>
             </div>
          </div>
        </div>
      )}
