        ...
        await addDoc(collection(db, "mail"), {
          ...
          message: {
            ...
            html: `<div style="font-family:sans-serif; background:#f4f4f4; padding:40px;">
                    <h2>Digital Entity Verdict</h2>
                    <p>${aiResponse.replace(/\n/g, '<br/>')}</p>
                    <br/>
                    <a href="https://calendly.com/motsumitl/30min" style="background:#FACC15; color:#000; padding:15px 25px; text-decoration:none; font-weight:bold; border-radius:8px;">BOOK RECOVERY CALL</a>
                   </div>`
          }
        });
      }
  ...
  const renderFormattedText = (text: string) => {
    ...
    return text.split('\n').map((line, i) => {
      if (line.includes('[SECTION]')) {
        return (
          <h4 key={i} className="text-yellow-500 font-black text-2xl uppercase mt-12 mb-4 border-b border-yellow-500/20 pb-2">
      ...
      if (line.includes('[FIX]')) {
        return (
          <div key={i} className="bg-yellow-500/5 border-l-4 border-yellow-500 p-6 my-6 italic text-white font-bold">
            {line.replace('[FIX]', 'STRATEGIC REQUIREMENT: ')}
          </div>
        );
      }
      ...
      const formatted = line.split(/(\[H\].*?|\*\*.*?\*\*)/g).map((part, j) => {
        if (part.startsWith('**')) {
          return (
            <strong key={j} className="text-white font-black underline uppercase">
              {part.replace(/\*\*/g, '')}
            </strong>
          );
        }
        if (part.startsWith('[H]')) {
          return (
            <span key={j} className="text-yellow-500 font-bold">
              {part.replace('[H]', '')}
            </span>
          );
        }
      ...
      return (
        <p key={i} className="mb-4 text-slate-300 leading-relaxed text-sm md:text-base">
          {formatted}
        </p>
      );
    });
        ...
        <div className="max-w-2xl mx-auto text-center animate-in fade-in zoom-in duration-500">
          ...
          <form onSubmit={(e) => { e.preventDefault(); setStep(2); }} className="space-y-4">
            <div className="relative">
              ...
              <input 
                required 
                className="w-full bg-slate-900 border border-slate-800 p-6 pl-14 rounded-2xl text-white outline-none focus:border-yellow-500" 
        ...
        <div className="max-w-xl mx-auto p-12 border border-slate-800 rounded-[3rem] bg-slate-900/40 shadow-2xl">
          ...
          <form onSubmit={runAudit} className="space-y-4">
            <div className="relative group">
              ...
              <input 
                required 
                className="w-full bg-slate-950 border border-slate-800 p-5 pl-12 rounded-2xl text-white outline-none focus:border-yellow-500" 
            ...
            <button 
              disabled={loading} 
              className="w-full bg-yellow-500 text-slate-950 py-6 rounded-2xl font-black uppercase tracking-widest flex items-center justify-center gap-2"
            >
        ...
        <div className="animate-in fade-in duration-1000">
          <div className="bg-yellow-500 p-10 rounded-[3rem] mb-16 text-slate-950 flex flex-col md:flex-row items-center justify-between gap-10 shadow-2xl">
            <div className="space-y-2 text-center md:text-left">
              ...
              <p className="font-bold text-lg italic opacity-80">
                Download the PDF below. A copy has been dispatched to {formData.email}.
              </p>
            </div>
            <button 
              onClick={downloadPDF} 
              className="bg-slate-950 text-white px-10 py-5 rounded-2xl font-black uppercase tracking-widest flex items-center gap-3 hover:scale-105 transition-transform"
            >
          ...
          <div ref={reportRef} className="p-16 border border-slate-800 rounded-[4rem] bg-slate-900/40 relative shadow-2xl">
            <div className="flex justify-between items-center mb-16 border-b border-slate-800 pb-12">
              <div className="space-y-1">
                <h3 className="text-yellow-500 font-black uppercase tracking-[0.5em] text-xs">
                  Smart Marketing // Protocol Analysis
                </h3>
            ...
            <div className="mt-32 p-16 bg-yellow-500 rounded-[3rem] text-slate-950 text-center">
              ...
              <p className="font-bold text-xl mb-12 max-w-xl mx-auto italic opacity-90 underline decoration-black/20">
                Secure your recovery call with Thabo today.
              </p>
              <a 
                href="https://calendly.com/motsumitl/30min" 
                className="bg-slate-950 text-white px-16 py-6 rounded-2xl font-black uppercase tracking-[0.2em] text-sm hover:scale-105 transition-transform inline-block"
              >
                Schedule Call
              </a>
            </div>
