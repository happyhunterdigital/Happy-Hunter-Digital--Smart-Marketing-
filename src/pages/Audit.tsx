...
export default function Audit() {
      ...
      if (db) {
        ...
        await addDoc(collection(db, "mail"), {
          ...
          message: {
            ...
            html: `<div style="font-family:sans-serif; background:#0f172a; color:#fff; padding:40px;"><h2>Digital Entity Verdict</h2><p>${aiResponse.replace(/\n/g, '<br/>')}</p><br/><a href="https://calendly.com/motsumitl/30min" style="background:#FACC15; color:#000; padding:15px; border-radius:8px; font-weight:bold; text-decoration:none;">SCHEDULE CALL</a></div>`
          }
        });
      }
      setStep(3);
  ...
  const renderText = (text: string) => {
    return text.split('\n').map((line, i) => {
      ...
      if (line.includes('[H]')) return <span key={i} className="text-yellow-500 font-black uppercase text-sm block mb-2">{line.replace('[H]', '')}</span>;
      
      const parts = line.split(/([A-Z]{5,})/g);
      return (
        <p key={i} className="mb-6 text-slate-300 leading-relaxed text-base font-medium">
      ...
      {step === 1 && (
        <div className="max-w-2xl mx-auto text-center animate-fade-in">
          ...
          <form onSubmit={(e) => { e.preventDefault(); setStep(2); }} className="space-y-4">
            ...
            <button type="submit" className="w-full bg-yellow-500 text-slate-950 py-6 rounded-3xl font-black text-xl shadow-2xl">Assess your Business</button>
          </form>
        </div>
      )}
      ...
      {step === 3 && (
        <div className="animate-fade-in">
          <div className="bg-yellow-500 p-10 rounded-[3.5rem] mb-12 text-slate-950 flex flex-col md:flex-row items-center justify-between gap-10 shadow-2xl">
            <div className="text-center md:text-left"><h3 className="text-4xl font-black uppercase leading-none italic">The Verdict is Ready</h3><p className="font-bold">Check your inbox. Download your master PDF below.</p></div>
            <button onClick={downloadPDF} className="bg-slate-950 text-white px-10 py-5 rounded-2xl font-black uppercase tracking-widest flex items-center gap-3 shadow-2xl hover:scale-105 transition-all text-xs"><Download size={20}/> Download PDF</button>
          </div>

          <div ref={reportRef} className="p-16 border-2 border-slate-800 rounded-[4rem] bg-slate-900/40 relative shadow-2xl overflow-hidden">
             <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-16 border-b-2 border-slate-800 pb-12 gap-8">
               <div className="space-y-2 text-left">
                 <h3 className="text-yellow-500 font-black uppercase tracking-[0.5em] text-[10px] animate-pulse"><Globe size={14} className="inline mr-2" /> Smart Marketing Graph Analysis</h3>
             ...
             <div className="max-w-4xl text-left">{renderText(result)}</div>
