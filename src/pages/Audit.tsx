...
import { Loader2, ShieldCheck, Mail, Phone, User, Download, Calendar, ArrowRight, Building2, MapPin, Globe, AlertTriangle } from 'lucide-react';
...
export default function Audit() {
  ...
  const downloadPDF = async () => {
    ...
    pdf.save(`Strategic-Audit-${formData.bizName}.pdf`);
  };
    ...
    <div className="pt-32 pb-20 px-6 max-w-6xl mx-auto min-h-screen">
      {step === 1 && (
        <div className="max-w-2xl mx-auto text-center animate-in fade-in zoom-in duration-500">
          ...
          <form onSubmit={(e) => { e.preventDefault(); setStep(2); }} className="space-y-4">
            ...
            <button type="submit" className="w-full bg-yellow-500 text-slate-950 py-6 rounded-3xl font-black text-xl hover:scale-95 transition-all">CONNECT TO GOOGLE MAPS</button>
          </form>
        </div>
      )}
      ...
      {step === 2 && (
        <div className="max-w-xl mx-auto p-12 border-2 border-slate-800 rounded-[3.5rem] bg-slate-900/40 shadow-2xl animate-in slide-in-from-bottom-10">
          ...
          <form onSubmit={runAudit} className="space-y-4">
            ...
            <button disabled={loading} className="w-full bg-yellow-500 text-slate-950 py-6 rounded-2xl font-black uppercase shadow-xl hover:bg-white">{loading ? <><Loader2 className="animate-spin" size={20}/> {loadingText}</> : "Generate Forensic Audit"}</button>
          </form>
        </div>
      )}
      ...
      {step === 3 && (
        <div className="animate-in fade-in">
          <div className="bg-yellow-500 p-12 rounded-[3rem] mb-16 text-slate-950 flex flex-col md:flex-row items-center justify-between gap-10 shadow-2xl relative overflow-hidden">
            <div className="relative z-10 space-y-3 text-center md:text-left max-w-xl">
              ...
              <p className="font-bold text-lg leading-tight italic">Download your report below. A master digital copy has also been dispatched to your email at <span className="underline">{formData.email}</span>.</p>
            </div>
            <button onClick={downloadPDF} className="bg-slate-950 text-white px-10 py-5 rounded-2xl font-black uppercase tracking-widest flex items-center gap-3 hover:scale-105 transition-all shadow-2xl">
              <Download size={24}/> Download Free PDF
            </button>
          </div>
          ...
          <div ref={reportRef} className="p-16 md:p-20 border-2 border-slate-800 rounded-[4rem] bg-slate-900/40 relative shadow-2xl overflow-hidden">
             <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-16 border-b-2 border-slate-800 pb-12 gap-8">
               <div className="space-y-1">
                 <h3 className="text-yellow-500 font-black uppercase tracking-[0.5em] text-[10px] flex items-center gap-2 animate-pulse"><Globe size={14}/> Forensic Analysis Protocol Active</h3>
             ...
             <div className="mt-32 p-16 bg-yellow-500 rounded-[3rem] text-slate-950 text-center relative overflow-hidden shadow-2xl">
               <div className="relative z-10">
                 ...
                 <p className="font-bold text-xl mb-12 italic opacity-90 max-w-xl mx-auto underline decoration-black/20 leading-relaxed">Your business is being filtered. Secure your recovery call today to claim your R16,300 value stack.</p>
