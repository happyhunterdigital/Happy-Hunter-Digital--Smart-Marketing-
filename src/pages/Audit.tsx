...
import { Search, Loader2, ShieldCheck, Mail, Phone, User, Download, ArrowRight, Globe } from 'lucide-react';
      ...
      {step === 3 && (
        <div className="animate-in fade-in">
          <div className="bg-yellow-500 p-10 rounded-[3.5rem] mb-12 text-slate-950 text-center space-y-4 shadow-2xl">
            ...
            <p className="font-bold text-lg">Check your email at {formData.email}. Download the master PDF below.</p>
            <button onClick={downloadPDF} className="bg-slate-950 text-white px-12 py-5 rounded-2xl font-black uppercase tracking-widest text-xs hover:scale-105 transition-transform flex items-center justify-center gap-3 mx-auto shadow-2xl"><Download size={20}/> Download PDF</button>
          </div>
          <div ref={reportRef} className="p-16 border-2 border-slate-800 rounded-[4rem] bg-slate-900/40 relative shadow-2xl overflow-hidden">
             ...
             <div className="mt-32 p-16 bg-yellow-500 rounded-[3rem] text-slate-950 text-center shadow-2xl">
               ...
               <a href="https://calendly.com/motsumitl/30min" className="bg-slate-950 text-white px-16 py-6 rounded-2xl font-black uppercase text-sm hover:bg-white hover:text-black transition-all shadow-2xl inline-block">Schedule Strategy Session</a>
             </div>
          </div>
        </div>
      )}
