...
export default function Audit() {
  ...
  const runAudit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStep(3);
    } catch (err) { setResult("SYSTEM_ERROR: Refused."); setStep(3); }
  ...
  const renderText = (text: string) => {
    return text.split('\n').map((line, i) => {
      ...
      if (line.includes('[FIX]')) return <div key={i} className="bg-yellow-500/10 border-l-4 border-yellow-500 p-8 my-10 text-white font-bold tracking-tight uppercase">STRATEGIC REQUIREMENT: {line.replace('[FIX]', '')}</div>;
  ...
  return (
    <div className="pt-32 pb-20 px-6 max-w-6xl mx-auto min-h-screen">
      ...
      {step === 2 && (
        <div className="max-w-xl mx-auto p-12 border-2 border-slate-800 rounded-[3.5rem] bg-slate-900/40">
          <h3 className="text-3xl font-black uppercase text-yellow-500 mb-8 text-center">Secure Results</h3>
          <form onSubmit={runAudit} className="space-y-4">
            <input required className="w-full bg-slate-950 border-2 border-slate-800 p-5 rounded-2xl text-white outline-none" placeholder="Full Name" value={formData.fullName} onChange={e => setFormData({...formData, fullName: e.target.value})} />
            ...
            <button disabled={loading} className="w-full bg-yellow-500 text-slate-950 py-6 rounded-2xl font-black uppercase">{loading ? <Loader2 className="animate-spin mx-auto" /> : "Initiate Smart Analysis"}</button>
          </form>
        </div>
      )}
      {step === 3 && (
        <div className="animate-in fade-in">
          <div className="bg-yellow-500 p-10 rounded-[3.5rem] mb-12 text-slate-950 text-center shadow-2xl shadow-yellow-500/20">
            <h3 className="text-3xl font-black uppercase leading-none italic">Verdict: Ready</h3>
            <p className="font-bold my-4">Download your free PDF below or check your inbox at {formData.email}.</p>
            <button onClick={downloadPDF} className="bg-slate-950 text-white px-10 py-4 rounded-2xl font-black flex items-center justify-center gap-3 mx-auto shadow-2xl"><Download size={20}/> Download PDF</button>
          </div>
          <div ref={reportRef} className="p-16 border-2 border-slate-800 rounded-[4rem] bg-slate-900/40 relative shadow-2xl">
            <div className="flex justify-between items-center mb-16 border-b-2 border-slate-800 pb-12">
              <div><h3 className="text-yellow-500 font-black uppercase tracking-[0.5em] text-[10px] animate-pulse">Smart Marketing Graph Analysis</h3><h4 className="text-5xl font-black text-white uppercase tracking-tighter leading-none">{formData.bizName}</h4></div>
            ...
            <div className="mt-32 p-20 bg-yellow-500 rounded-[3rem] text-slate-950 text-center relative overflow-hidden">
               ...
               <a href="https://calendly.com/motsumitl/30min" className="bg-slate-950 text-white px-16 py-6 rounded-2xl font-black uppercase text-sm hover:bg-white transition-all shadow-2xl">Schedule Recovery Call</a>
            </div>
          </div>
        </div>
      )}
