...
export default function Audit() {
  ...
  const runAudit = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    setLoadingText("Querying Smart Marketing Graph..."); // BRANDED LOADING
  ...
  const downloadPDF = async () => {
    ...
    pdf.save(`Strategic-Verdict.pdf`);
  };
  ...
  const renderText = (text: string) => {
    return text.split('\n').map((line, i) => {
      ...
      if (line.includes('[FIX]')) return <div key={i} className="bg-yellow-500/10 border-l-4 border-yellow-500 p-8 my-10 text-white font-bold tracking-tight uppercase">Requirement: {line.replace('[FIX]', '')}</div>;
      const parts = line.split(/([A-Z]{4,})/g);
      return <p key={i} className="mb-6 text-slate-300 leading-relaxed text-lg">{parts.map((part, j) => /^[A-Z]{4,}$/.test(part) ? <span key={j} className="text-yellow-500 font-black">{part}</span> : part)}</p>;
    });
  };
    ...
    <div className="pt-32 pb-20 px-6 max-w-6xl mx-auto min-h-screen">
      ...
      {step === 2 && (
        <div className="max-w-xl mx-auto p-12 border-2 border-slate-800 rounded-[3.5rem] bg-slate-900/40 shadow-2xl animate-in slide-in-from-bottom-10 text-center">
          <h3 className="text-2xl font-black uppercase text-yellow-500 mb-8">Secure Results</h3>
      ...
      {step === 3 && (
        <div className="animate-in fade-in">
          <div className="bg-yellow-500 p-10 rounded-[3.5rem] mb-12 text-slate-950 text-center space-y-4 shadow-2xl shadow-yellow-500/20">
            ...
            <button onClick={downloadPDF} className="bg-slate-950 text-white px-10 py-4 rounded-2xl font-black flex items-center justify-center gap-3 mx-auto shadow-2xl"><Download size={20}/> Download Free PDF</button>
          </div>
          <div ref={reportRef} className="p-20 border-2 border-slate-800 rounded-[4rem] bg-slate-900/40 shadow-2xl overflow-hidden">
             ...
             <div className="mt-32 p-20 bg-yellow-500 rounded-[3rem] text-slate-950 text-center shadow-2xl">
               ...
               <a href="https://calendly.com/motsumitl/30min" className="bg-slate-950 text-white px-16 py-6 rounded-2xl font-black uppercase text-sm hover:bg-white hover:text-black transition-all shadow-2xl">Schedule Strategy Call</a>
             </div>
          </div>
        </div>
      )}
