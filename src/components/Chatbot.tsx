...
const getFallbackResponse = (input: string): string => {
  ...
  return 'I\'m currently operating in limited mode. For immediate assistance, please run the Entity Scan tool or email hello@happyhunterdigital.com.';
};
...
${history.slice(-4).map((h, i) => `${i % 2 === 0 ? 'User' : 'Hunter'}: ${h}`).join('\n')}
...
Hunter AI (respond in 1-2 sentences, authoritative tone, mention "Entity Scan" if relevant):`;
  ...
  return (
    <>
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-6 z-50 w-80 max-w-[calc(100vw-3rem)] bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="h-72 overflow-y-auto p-3 space-y-3 bg-slate-950/30">
            ...
            {loading && (
              <div className="flex justify-start">
                <div className="bg-slate-800 p-2 rounded-xl"><Loader2 className="animate-spin text-yellow-500" size={14} /></div>
              </div>
            )}
          ...
          <div className="p-3 border-t border-slate-800 bg-slate-900">
            <div className="flex gap-2">
              ...
              <button
                ...
                className="bg-yellow-500 text-slate-950 p-2 rounded-lg disabled:opacity-50 hover:bg-yellow-400"
              >
                <Send size={14} />
              </button>
            </div>
