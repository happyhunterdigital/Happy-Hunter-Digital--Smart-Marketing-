      ...
      {isOpen ? (
        <div className="w-80 md:w-96 h-[550px] bg-slate-950 border border-slate-800 rounded-[2rem] shadow-2xl flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-10">
          <div className="p-5 bg-yellow-500 text-slate-950 flex justify-between items-center font-bold">
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col leading-none">
                ...
                <span className="text-[8px] uppercase tracking-widest opacity-70">Entity Intelligence Active</span>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            {loading && <div className="text-[10px] text-yellow-500 font-bold animate-pulse uppercase tracking-widest">Analyzing...</div>}
            <div ref={scrollRef} />
          </div>
          <div className="p-4 border-t border-slate-900 flex gap-2">
            <input className="flex-1 bg-slate-950 p-4 rounded-xl text-xs outline-none border border-slate-800 focus:border-yellow-500 text-white" value={input} onChange={(e) => setInput(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && sendMessage()} placeholder="Ask the ecosystem..." />
        ...
        <button onClick={() => setIsOpen(true)} className="bg-yellow-500 text-slate-950 p-5 rounded-full shadow-[0_0_30px_rgba(250,204,21,0.3)] hover:scale-110 transition-all border-4 border-slate-950 relative">
