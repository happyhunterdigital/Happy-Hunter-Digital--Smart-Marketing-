...
import { getAiResponse } from '../lib/gemini'; // Changed from hunterAI to getAiResponse
...
export default function Chatbot() {
  ...
  const [messages, setMessages] = useState([
    { role: 'bot', text: "Signal established. I am Hunter AI. Is your entity invisible to the smart filter?" }
  ]);
  ...
  async function sendMessage() {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'bot', text: "Signal drop. Book a call: https://calendly.com/motsumitl/30min" }]);
    } finally {
      setLoading(false);
    }
  }
      ...
      {isOpen ? (
        <div className="w-80 md:w-96 h-[550px] bg-slate-950 border border-slate-800 rounded-[2rem] shadow-2xl flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-10">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] p-4 rounded-2xl text-xs leading-relaxed ${m.role === 'user' ? 'bg-yellow-500 text-slate-950 rounded-tr-none' : 'bg-slate-900 text-slate-300 border border-slate-800 rounded-tl-none'}`}>
                  {m.text}
                </div>
              </div>
            ))}
          ...
          <div className="p-4 border-t border-slate-900 flex gap-2">
            ...
            <button onClick={sendMessage} className="bg-yellow-500 p-4 rounded-xl text-slate-950"><Send size={18}/></button>
          </div>
        </div>
      ) : (
