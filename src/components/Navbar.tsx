...
import { Menu, X, Zap, Globe, BookOpen, ShieldCheck, HelpCircle, User, Cpu } from 'lucide-react';
...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    { name: 'Intelligence', path: '/blog/digital-authority-architecture', icon: <Cpu size={14} /> }, // THE BLOG ANCHOR
  ...
  return (
    <>
        ...
        <nav className="bg-slate-900/60 backdrop-blur-xl border border-slate-800 rounded-full px-6 py-3 shadow-2xl flex items-center gap-10 transition-all hover:border-yellow-500/30">
          ...
          <div className="hidden lg:flex items-center gap-6 text-[10px] font-black uppercase tracking-widest text-slate-400">
            {navLinks.map((link) => (
              ...
              >
                <span className="group-hover:animate-pulse text-yellow-500/50">{link.icon}</span>
                {link.name}
              </Link>
            ))}
          </div>
      ...
      <div className={`fixed inset-0 z-[90] lg:hidden transition-all duration-500 ${isMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className={`absolute right-0 top-0 h-full w-[85%] max-w-sm bg-slate-900 border-l border-slate-800 p-10 flex flex-col justify-center gap-12 transition-transform duration-500 ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          
          <div className="space-y-6">
            <p className="text-[10px] font-black uppercase tracking-[0.5em] text-slate-700 border-b border-slate-800 pb-4">Entity Protocol v2.0</p>
          ...
          <button onClick={closeMenu} className="absolute top-10 right-10 text-slate-500 hover:text-white transition-colors">
            <X size={32} />
          </button>
        </div>
      </div>
