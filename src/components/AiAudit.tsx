  ...
  const handlePhoneChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (input.length > 3) {
      ...
      if (result.isValid) {
        ...
        if (isMobile) {
          setPhoneStatus({ isValid: true, message: `✓ Valid WhatsApp Number (${result.country})` });
        } else {
        ...
        <form onSubmit={(e) => { e.preventDefault(); runForensicScan(); }} className="space-y-6 bg-gray-900/40 p-10 rounded-[2.5rem] border border-yellow-500/20 backdrop-blur-xl animate-fade-in shadow-2xl">
          ...
          <div className="space-y-4 mb-6">
            ...
            <div className="relative pt-2">
              ...
              />
              {phoneStatus.message && (
                <p className={`text-xs mt-2 font-medium ${phoneStatus.isValid ? 'text-green-500' : 'text-red-500'}`}>
                  {phoneStatus.message}
                </p>
              )}
            </div>
