      ...
      {step === 4 && verdict && (
        <div className="space-y-8 animate-fade-in pb-10">
          <div ref={reportRef} className="p-8 md:p-12 bg-[#050505] border border-gray-800 rounded-[3rem] shadow-2xl relative overflow-hidden">
            ...
            {verdict.score >= 70 ? (
              <div className="mb-10 p-6 bg-green-500/10 border border-green-500/20 rounded-2xl">
                <h3 className="text-green-500 font-black uppercase tracking-tight flex items-center gap-2 mb-2"><CheckCircle size={18} /> Entity Verified: Strong Baseline</h3>
                <p className="text-gray-300 text-sm leading-relaxed">Congratulations. Your traditional SEO and Google Maps foundation is solid. However, standard search is evolving rapidly. To prevent competitors from overtaking you in AI-driven search (ChatGPT, Gemini, SGE), you must upgrade from basic SEO to Generative Engine Optimization (GEO).</p>
              </div>
            ) : (
              <div className="mb-10 p-6 bg-orange-500/10 border border-orange-500/20 rounded-2xl">
                <h3 className="text-orange-500 font-black uppercase tracking-tight flex items-center gap-2 mb-2"><AlertTriangle size={18} /> Critical Vulnerability Detected</h3>
                <p className="text-gray-300 text-sm leading-relaxed">Your digital architecture is actively repelling algorithms. You are experiencing the "Ghost Effect"—meaning high-intent customers searching for your services are being routed directly to your competitors. Immediate intervention is required.</p>
              </div>
            )}

            {/* RAW TELEMETRY DASHBOARD INJECTED HERE */}
            {verdict.telemetry && (
              <div className="mb-10">
                <h3 className="text-gray-500 font-black uppercase text-xs tracking-widest mb-4 flex items-center gap-2"><Database size={16}/> Raw Footprint Extraction</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="bg-[#0a0a0a] border border-gray-800 p-4 rounded-xl shadow-inner">
                    <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mb-2">Google Maps Node</p>
                    <p className={`font-black text-sm uppercase ${verdict.telemetry.mapsStatus === 'VERIFIED' ? 'text-green-500' : 'text-red-500'}`}>{verdict.telemetry.mapsStatus}</p>
                  </div>
                  <div className="bg-[#0a0a0a] border border-gray-800 p-4 rounded-xl shadow-inner overflow-hidden">
                    <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mb-2">Website Root URL</p>
                    <p className="font-bold text-sm text-white truncate">{verdict.telemetry.website}</p>
                  </div>
                  <div className="bg-[#0a0a0a] border border-gray-800 p-4 rounded-xl shadow-inner">
                    <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mb-2">AI Schema (JSON-LD)</p>
                    <p className={`font-black text-sm uppercase ${verdict.telemetry.schema ? 'text-green-500' : 'text-red-500'}`}>{verdict.telemetry.schema ? 'DETECTED' : 'MISSING'}</p>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="space-y-10">
              <div>
                <h3 className="text-yellow-500 font-black uppercase text-xs tracking-widest mb-4 flex items-center gap-2"><ShieldCheck size={16}/> Forensic AI Summary</h3>
                <p className="text-white text-lg font-medium leading-relaxed italic border-l-4 border-gray-800 pl-4">"{verdict.summary}"</p>
              </div>
              <div className="grid gap-4">
                <h3 className="text-gray-500 font-black uppercase text-xs tracking-widest mb-2 flex items-center gap-2"><XCircle size={16}/> Specific Technical Weak Spots</h3>
            ...
            <div className="mt-12 p-8 md:p-10 bg-gradient-to-br from-[#0a0a0a] to-black border border-yellow-500/30 rounded-[2rem] text-center relative overflow-hidden">
              ...
              <p className="text-gray-400 mb-8 text-sm md:text-base max-w-xl mx-auto leading-relaxed">Book a Free 30-Minute Discovery Call with <strong>Thabo</strong>, Head of happyhunterdigital. We will review this exact report together and map out your custom Recovery Protocol.</p>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <a href={`https://wa.me/27601016673?text=Hi%20Thabo!%20I%20just%20completed%20the%20Survival%20Scan%20for%20${form.biz}%20and%20scored%20${verdict.score}/100.%20Let's%20talk%20about%20my%20Recovery%20Protocol.`} target="_blank" rel="noreferrer" className="w-full p-5 bg-[#0a0a0a] border border-gray-800 text-white rounded-2xl font-bold uppercase text-xs hover:text-yellow-500 hover:border-yellow-500 transition-all flex items-center justify-center gap-3">
