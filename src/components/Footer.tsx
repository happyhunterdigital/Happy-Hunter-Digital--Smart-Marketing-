      ...
      <div className="max-w-7xl mx-auto grid md:grid-cols-2 lg:grid-cols-4 gap-16">
        ...
        <div className="space-y-6 text-left">
          ...
          <div className="flex flex-wrap gap-3">
            {socials.map((soc, i) => (
              <a key={i} href={soc.url} target="_blank" rel="noreferrer" title={soc.name} className="p-3 bg-slate-900 border border-slate-800 rounded-xl text-slate-500 hover:text-yellow-500 hover:border-yellow-500/30 transition-all">
                {soc.icon}
              </a>
            ))}
          </div>
        ...
        <div className="space-y-6 text-left">
          ...
          <div className="space-y-3">
             <Link to="/faq" className="block text-slate-600 hover:text-slate-400 text-[10px] font-black uppercase tracking-widest">Privacy & POPI Act</Link>
