...
export const functions = getFunctions(app, "africa-south1"); // Johannesburg Region

// --- SECURE PROXY HANDSHAKES ---
...
export const performAuditAnalysis = async (bizName: string, location: string) => {
  try {
    ...
    return result.data; // RETURNS STRUCTURED JSON
  } catch (err) {
    console.error("Audit Protocol Refused");
    return null;
  }
};
...
export const callHunterAI = async (prompt: string) => {
  try {
    ...
    return "Handshake failed. Connect with Thabo directly for manual audit.";
  }
};
